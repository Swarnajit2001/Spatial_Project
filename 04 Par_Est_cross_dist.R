...
load('03 We_Did_It.Rdata')
load("04 Par_Est_cross_dist.Rdata")
...
for(i in 1 : P){
  foo <- evd::qgev(p, loc = final.par.est.gev[i,3], scale = final.par.est.gev[i,2], shape = final.par.est.gev[i,1])
  foo <- foo / max(foo)
  quant4 = c(quant4, foo)
}
...
combined_plot.2 <- wrap_elements(grid::textGrob("Emprirical quantiles",
                                                ...
                                                x = 0.3, 
                                                y = 0.47, 
                                               gp = gpar(fontsize = 17))) + combined_plot.1  +
